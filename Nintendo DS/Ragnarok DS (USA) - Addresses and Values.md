...
### Byte Layout
#### Equipment Slot Offsets (from Character Base)
```
...
#### Equipment Field Offsets (from Slot Entry)
```
...
### Notes
- Offsets are applied in order: Character Base → Equipment Slot → Field.
> **Example:**<br> Value `0x0020F9E0` (Ales)<br>+ Offset `0x1C` (Cape)<br>+ Sub-Offset `0x02` (Refinement Lv.)<br>= `0x0020F9FE` (Ales' Cape Refinement Lv.)
...
## Character Customization (`0x104` bytes each)
```
...
## Other Addresses
```
Party Slot Map (24-bit):
  0x001eb9e0 - Current Party              0x001eb9e3 - Previous Party

Mirage Tower:
  0x001ecfa4 - Name (14 bytes)            0x001ecfbf - Customization (24-bit)
...
- The **Party Slot Map** does *not* directly change your active party. Instead, it determines which **Guild Roster slots** your current party members occupy when exiting the Party Formation menu.
  ...
  - Attempting to duplicate Ales (`$00`) is not advised. His clone behaves as if awaiting input from Player 2 or Player 3, and will not act on his own.
...
### Other Addresses
```
Party Slot Map (both): $00–$0F, $FF (Empty)

Mirage Tower Name: $30–$7D

Mirage Tower Customization: 
  +0x00 -     Gender: $00–$01           +0x01 - Hair Style: $00–$16
  +0x02 - Hair Color: $00–$07

Zeny: $00000000–$3B9AC9FF

Status Points: $0000–$03E7              Skill Points: $00–$59
```

### Notes
- For Equipment, some Expected Value Ranges are fixed values because the game only expects one category of equipment to be in that slot. (`$11` for Hats, `$15` for Accessories, et cetera.)

- For Mirage Tower Customization data, Hair Style `$16` appears to be unused and cannot normally be unlocked.
